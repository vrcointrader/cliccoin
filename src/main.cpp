...
uint256 hashGenesisBlock("0x6b8f9a4d765dbe645bb11ef1e120701931c8d92ec412377fd148e3d8e4c5ea42");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // cliccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "cliccoin Signed Message:\n";
...
{
    // cliccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 22105200 * 5  / 100 / 50;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // cliccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // cliccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // cliccoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6b8f9a4d765dbe645bb11ef1e120701931c8d92ec412377fd148e3d8e4c5ea42");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1405491981;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084835780;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("cliccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("cliccoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running cliccoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("cliccoinMiner terminated\n");
        throw;
    }
}
